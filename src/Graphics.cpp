...
void Graphics::changeGameSpeed() {
    ...
    if (simulations_stats) {
        ...
        auto duration = elapsed_real_time * game_time_speed.load(); // .load() lecture atomique sécurisée
...
void Graphics::updateEntities(bool draw){
    ...
    for (Entity* e : entities) {
        // Marquage pour suppression différée (on ne supprime jamais directement dans une boucle itérative)
        if(e->getHp() <= 0){
        ...
        // Optimisation : Si on accélère le temps (x50), on ne dessine pas à chaque étape de calcul
        if (!draw) return;

        e->draw(renderer);
        if(showHealthBars) e->drawHealthBar(renderer);

        // Affichage de la Hitbox rouge pour l'entité sélectionnée
...
void Graphics::updateProjectiles(bool draw){
    ...
    // Liste locale pour noter les projectiles à détruire après calcul
    ...
    for (Projectile* proj : projectiles_snapshot) {
        ...
        for (Entity* e : entities) {
            if(owner != nullptr && e != owner && e->getHp() > 0){
                ...
                // Comparaison des distances au carré (évite sqrt, très coûteux en CPU)
                if ((dx*dx + dy*dy) <= (sumRadius * sumRadius)) {
                    e->setHp(e->getHp() - (int)damage);
                    hit = true;
                    break; // Un projectile ne touche qu'une seule cible
                }
            }
        }
    ...
    // On reprend le verrou pour modifier le vecteur 'projectiles' partagé
...
void Graphics::update(bool* running, bool* keep_playing) {
    ...
    handleEvent(running, keep_playing); // Gestion clavier/souris
    ...
    for(int i = 0; i<game_time_speed; i++) { // On répète l'action plusieurs fois si le temps est plus rapide
        ...
        if (entities.size() <= min_number_entity){
            ...
            deleteAllProjectiles();
            generation++;
            simulations_stats->updateLastGeneration(entities.size());             // Statistiques

            std::vector<Entity*> new_entities;
            std::vector<Entity*> parents_left = entities;

            // Tant qu'on a des paires de parents
            while (parents_left.size() >= 2) {
                // Nombre d'enfants aléatoir
                ...
                if (same_type_peace) {
                    ...
                    if (i == parents_left.size()) { // Aucun parent n'a le même type que celui-ci
            ...
            // Ajout de la nouvelle génération au monde
    ...
    // Affichage de l'interface par-dessus le jeu
    global_mutex.lock();
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN) {
            ...
            if (event.button.button == SDL_BUTTON_LEFT) {
                ...
                //On vérifie si le clique est sur une entité?
                std::lock_guard<std::mutex> lock(global_mutex);
...
Entity* Graphics::createNewEntityFromParents(Entity* e1, Entity* e2){
    ...
    // Détermine le type de l'enfant (Guerrier, Mage...) avec risques de mutation
    Entity* new_entity = instantiateChildByType(e1, e2);

    // Placement aléatoire (Spawn)
    int width, height;
    SDL_GetWindowSize(getWindow(), &width, &height);
    int x = std::rand() % (width-50) + 50;
    int y = std::rand() % (height-50) + 50;
    new_entity->setX(x);
    new_entity->setY(y);

    // HÉRITAGE DES STATISTIQUES
    // Chaque stat est une combinaison de celles des parents + mutation potentielle
    ...
    new_entity->updateAttackCooldown(); // Important : recalculer le cooldown selon les nouveaux dégâts
...
Entity* Graphics::instantiateChildByType(Entity* e1, Entity* e2){
    ...
    // 50% de chance d'hériter du type du père ou de la mère
    ...
    if ((std::rand() % 100) < mutationTypeRate) { // 15% de chance de muter
        ...
        // Choix aléatoire parmi les types restants
        if (std::rand() % 2 == 0){
...
float Graphics::calculateNewAttribute(float value1, float value2){
    // Moyenne pondérée aléatoire : V = A*x + B*(1-x)
    // Cela permet d'avoir une valeur située quelque part entre le père et la mère
    float X = (std::rand() % 101) / 100.0f;
    float new_value = (value1 * X) + (value2 * (1.0f - X));

    // Mutation de Statistique
    if ((std::rand() % 100) < mutationStatsRate) {
        // Variation de +/- 0% à 20%
        float variation = (std::rand() % 21) / 100.0f;
        if (std::rand() % 2 == 0) variation = -variation; // Signe aléatoire
